...
当前状态：工具调度与文件工具已实现，通过 `safe_path` 做路径验证，但是否完整委托到 WorkspaceAuthority 仍需验证。`edit_file` 已升级为**批量事务模式**：接受 `edits: [{search, replace}]` 数组，在内存影子缓冲区中逐项验证并应用替换，全部成功后才统一落盘写入。引入**二级符号归一化兜底**（统一换行符、剥离行尾空格），消除跨平台匹配差异。任一 `search` 匹配失败即事务熔断回滚，不执行任何磁盘写入。`read_file` 已支持 `start_line`/`end_line` 参数进行局部视窗读取，每行返回时前缀行号，头部附带元数据提示（文件路径、视窗范围、总行数）。

工具层当前主要风险：文件访问越权测试不足；bash 是否严格受 workspace 限制需验证；`read_file` 局部视窗的 Token 收益尚未验证（v6 基线显示反增）；`edit_file` 的错误消息缺乏指导性；工具 `success` 语义可能需要拆分。
