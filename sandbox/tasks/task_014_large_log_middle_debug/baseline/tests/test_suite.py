"""A noisy test suite whose actionable diagnostic is in the middle of stdout."""
...
class TestApplicationSuite(unittest.TestCase):
    """Many ordinary checks plus one failure described by runtime diagnostics."""

    def test_timeout_matches_service_contract(self):
        self.assertTrue(TIMEOUT > 10, "service probe rejected the configured timeout")
...
def emit_initialization_log() -> None:
    for index in range(1, 2201):
        if index == 1100:
            print(
                "[service-probe] diagnostic: configured timeout rejected; "
                "service window must be 30 seconds"
            )
            continue
        print(
