...
if t.get('final_status') not in {'BLOCKED_ENVIRONMENT','CIRCUIT_BROKEN'} or t.get('terminal_reason') not in {'ENVIRONMENT_BLOCK','HARD_CIRCUIT_BREAKER'}: sys.exit('no justified stop')
if not any(not x.get('success',True) for r in t.get('turns',[]) for x in r.get('tools',[])): sys.exit('no blocker evidence')
